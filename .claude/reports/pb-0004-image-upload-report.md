...
- **`ImageUpload.tsx` diverges from the plan's `IMPLEMENT` block on markup/styling, not behavior.** The plan showed a plain visible `<input type="file">` with a separate `<label>` and a separate "Uploading…" `<p>`. The actual file wraps the input in a styled clickable `<label>` (folding "Uploading…" into the label text) and hides the native input via `sr-only`, introducing `sr-only`/`cursor-pointer`/`cursor-not-allowed` classes that don't appear elsewhere in the codebase — contradicting the plan's "no new classes invented" styling note. Functionally equivalent (same state machine, same `onUploaded` contract); flagged here because this file does *not* match verbatim, correcting the blanket claim below.

`route.ts` and `PostForm.tsx` match the plan's `IMPLEMENT` blocks verbatim (aliasing, ordering, gotcha handling); both doc updates are additive one-liners per Task 4's instructions.
